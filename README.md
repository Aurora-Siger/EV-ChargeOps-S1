...
- **Física:** carregador GoodWe HCA G2, com autenticação por RFID e medição pelo medidor MID integrado - interfaces detalhadas em "Carregador GoodWe HCA G2 - interfaces disponíveis".
- **Conectividade:** GoodWe Cloud (SEMS Portal/SEMS+), ponte entre o HCA G2 e o backend, mais as APIs externas Open Charge Map e Google Places - endpoints detalhados em "API GoodWe SEMS Portal - endpoints mapeados" e "2.1 Aprofundamento - Mapeamento de APIs complementares".
- **Aplicação:** backend Python (FastAPI, PostgreSQL, Redis) - ingestão e normalização da sessão, motor de rateio, motor de IA e geração de faturas, detalhados em "Fluxo de dados - da sessão à fatura" e "Modelo de rateio proposto".
- **Apresentação:** aplicação web React para gestor e morador (histórico, faturas, alertas, recomendações e notificações) - fluxo completo em "Fluxo de dados - da sessão à fatura".
